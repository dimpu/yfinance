...
// Service provides quote data retrieval.
type Service struct {
...
// Get fetches real-time or near real-time quote data for the given symbols.
...
func (s *Service) Get(ctx context.Context, symbols []string, opts *Options) ([]Quote, error) {
	if len(symbols) == 0 {
	...
	body, err := s.fetcher.Fetch(ctx, fetch.FetchConfig{URL: u, NeedsCrumb: true})
	if err != nil {
		return nil, err
	}
